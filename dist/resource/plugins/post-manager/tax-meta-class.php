...
class Tax_Meta_Class {
  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme']))
      if ($meta_box['use_with_theme'] == true){
        ...
        $this->SelfPath = plugins_url( 'mhv', plugin_basename( dirname( __FILE__ ) ) );
      }else{
    ...
    add_action('delete_term', array($this,'deletetaxonomy_metadata'), 10,2);
  }
  ...
  public function load_scripts_styles() {
    ...
    if (in_array($taxnow,$this->_meta_box['pages'])){
      ...
      //wp_enqueue_style( 'tax-meta-class',plugins_url('/css/tax-meta-class.css',__FILE__) );
      // Enqueue Meta Box Scripts
      wp_enqueue_script( 'tax-meta-class',plugins_url('/js/tax-meta-class.js',__FILE__), array( 'jquery' ), null, true );
  ...
  */
  ...
  public function check_field_color() {
    ...
    if ( $this->has_field( 'color' ) && $this->is_edit_page() ) {
      ...
      wp_enqueue_style( 'farbtastic1' );
      wp_enqueue_script( 'farbtastic1' );
    }
