...
function add_sortable_script(){
	...
	if ( current_user_can('edit_others_pages') && $screen->id == 'edit-'.$posttype && isset( $wp_query->query['orderby'] ) && $wp_query->query['orderby'] == 'menu_order title' ) {
		wp_enqueue_script( 'script_post_ordering', get_template_directory_uri() . '/admin-themes/js/post-ordering.js', array('jquery-ui-sortable'), '1.0', true );
	}
}
add_action("admin_head","add_sortable_script");
function jam_default_sorting_link( $views ) {
	...
	$views['byorder'] = '<a href="'. $query_string . '" class="' . $class . '">Sắp xếp</a>';
	return $views;
}

add_filter( 'views_edit-'.$posttype, 'jam_default_sorting_link' );
